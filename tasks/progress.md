...
## TODO（下一輪，建議新 chat）
- [ ] **整站 Nacre Ledger 遷移**（使用者明確要求）：App.jsx / MyNotes.jsx / ConnectButton chrome 統一成 leaderboard 的 obsidian+nacre 風格；順手把 deferred 項目清掉（`<meta charset>` 排序、2 個 hardcoded hex `#9c8045`/`#c9c1e8` → theme token、MyNotes 的 `getNotes(account.address)` 同款 address-normalization gap）。
