...
## Current Task
**leaderboard 前端視圖（"Nacre Ledger"）✅ 完成、已 merge 進 local main、未 push（2026-06-22）**：公開 issuer 排行榜（按 realized PnL），連錢包前可見、連上後 `normalizeSuiAddress` 兩邊比對高亮自己那行（`YOU` 標）。後端零改動（`/leaderboard` query 早已存在）。**3 檔新增/改**：`frontend/src/{Leaderboard.jsx,Leaderboard.css,theme.css}` + `api.js#getLeaderboard` + `index.html`(Fraunces+Martian Mono) + `App.jsx` mount（在 `{account &&}` guard 外）。**美學 = PearlFoundry brand 拆義素**：obsidian 黑曜底 + 珍珠母 nacre 虹彩 + 熔金 rank pip；Fraunces serif + Martian Mono tabular-nums；staggered reveal + prefers-reduced-motion；拒絕 AI-slop（無 Inter/紫漸層/generic card）。**範圍決策**：本輪共享 theme tokens 但只套 Leaderboard；**下一輪把整站（App/MyNotes/ConnectButton）遷移成 Nacre Ledger 統一風格（使用者明確要求）**。**SDD 2 task（fresh subagent + spec/quality 雙驗）+ final whole-branch review = opus Ready-to-merge YES + sui-frontend CONDITIONAL PASS**。Fix wave（commit 89a0991）：guard null issuer（normalize/slice crash→blank board）、`getLeaderboard` Array.isArray 防護、empty-state `&& !loading` 去閃爍。**Deferred 到下輪 migration**：`<meta charset>` 排序、2 個 hardcoded hex(#9c8045/#c9c1e8→token)、pnl===0 不顯示`+`(刻意)。spec `docs/superpowers/specs/2026-06-22-leaderboard-frontend-design.md`、plan `…/plans/2026-06-22-leaderboard-frontend.md`。**merge commit `da44fe6`（--no-ff），local main 領先 origin/main 4 commits、未 push**——等你決定 remote 整合（上次是 ff push 到 origin/main）。`vite build` green。**下一輪候選：整站 Nacre Ledger 遷移 / sponsored-tx gas station / payoff diagram / 其他 strategy。**
