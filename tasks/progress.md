...
## TODO（下一輪，建議新 chat）
- [x] **🔴 UI 微調 + 2 個 live bug ✅ 完成（2026-06-24，未 commit）— 見 Current Task**：6 項（claim oracle asset bug、mint InsufficientGas、compact→full 圖、settled PnL、win-rate 置中、address+button 同行靠右）。**未 commit、未 push、等使用者**。
- [ ] **🔴 下一輪：UI 調整續**。剩餘候選：mint 預覽 x 軸窄 band 兩端都顯示 `63k`（k-formatting 解析度不夠，待決定是否加小數）；payoff 圖 forward 紅虛線在窄圖可見度。
...
## Blockers
- （無）。**2026-06-24 UI 微調 6 項全未 commit**（working tree 改了 7 檔，見 Current Task）。payoff diagram + 4 live-run fix 已在 local `main`（tip `5c4f88d`），**未 push**（等使用者）。前一 push 點 `origin/main` = `15baf13`。
- **後端 server 跑在背景**（`server.js indexer.db 8787`，session 結束可能被收）；只服務 HTTP **不跑 poller**，新 mint note 不自動 ingest，需另跑 `node scripts/indexer/ingest.js indexer.db`。
