...
## 2026-06-25 — Mascot accents：cache 優化會生出「視覺對但 code 騙人」的 dead element；codex 每 pass 吐新主觀 nit 要設收斂線；live 比較頁是 treatment 決策的對的工具
- **plan 的 micro-optimization 會製造 reviewer 抓得到的 confusing code**：masthead 彩蛋輪播 plan 寫 `mascotIdx===0 ? '/logo-mark.png' : mascotSrc(MASCOT_CYCLE[mascotIdx])`（I4 cache 優化：初始用已快取的 logo-mark.png 免重複 fetch）。但 `MASCOT_CYCLE[0]=3` 因 idx 0 特例化而**永不經 mascotSrc**＝dead array element，註解「3→1→2」也半誤導。**opus 全分支 review 沒抓到**（它認定視覺循環對就放行），**codex correctness lens 第 3 pass 才抓**＝off-by-one/dead element。真相：logo-mark.png 與 logo_3-clear.png **byte-identical**，所以視覺確實 3→1→2 沒 bug，但 code 騙人。**教訓**：(1) 為省一次 130KB fetch 引入的特例分支，若讓「具名/陣列結構」與「實際 render」對不上，clarity 損失 > 優化收益——直接統一走 `mascotSrc(MASCOT_CYCLE[idx])`（cycle 全用 -clear 系列反而徹底消除重複 URL）。(2) 「視覺對」不等於「code 對」；reviewer 會抓結構性 dead code 即使無 runtime bug，別用「跑起來對」搪塞。
- **codex --multi 每個 pass 會生出新的主觀 readability nit，必須自設收斂線**：dual-review 跑了 5 passes。correctness/security 從第 1 pass 就全程 LGTM，真 finding 在 R1–R3（dead reveal abstraction、Mascot.css 自包含、cycle bug）；R4/R5 開始吐「magic number 改具名常數」「empty-state CSS 該搬離 App.css」「資產改語意檔名」這類**每 pass 換一個的主觀意見**，且有的與專案慣例衝突（App.css 本就是 global sheet、numbered 檔名對應 docs 源檔＝Rule 11）。**教訓**：dual-review 的價值在 correctness/security + 前幾輪真 finding；當連續 pass 只剩「換湯不換藥的 taste nit + 與既有慣例衝突項」就該**收斂 ship**（呼應 dev-rules 同錯 3 次停手 / 反無限 retry）。裁決三態要明確：真 bug→修、與慣例衝突→reject 附理由(Rule 7/11)、純 taste 且便宜又真清晰→做一個了結（這次只挑「具名常數消除 variant={3} opacity」做，其餘 reject）。別被 codex 的「再 review 又有新意見」拖進無限迭代。
- **treatment/視覺風格的 A/B 抉擇，做一個真 live 比較頁讓使用者選，勝過文字描述**：kawaii 彩色卡通 vs refined 品牌的整合，关键是「saturate(0.92) 等於沒做」這種只有眼睛看得出的事。brainstorming 時建 `treatment-compare.html`（真圖 base64 內嵌+真品牌底色+真 CSS filter，A 灰階 hover-reveal / B nacre 雙調 / mint 峰值 / 空狀態合成並排），使用者一看就選「混合：B 空狀態 always-on + A masthead 互動 reveal」（解決 A 在手機無 hover、B 無互動回饋 各自的短板）。**教訓**：(1) 資產一律 base64 內嵌（延續 06-23：relative path 在 file:// 會破圖）。(2) 純 CSS filter 效果（saturation/grayscale/blend）文字描述無感，真頁面一目了然；治 slop 的 treatment 強度更是只有看真圖才判得準。
- **2 路 plan-stage design review 在「還沒寫 code」就抓到非法 HTML + 既有 keyframe 碰撞 + 缺 render site**（再次印證 plan-stage review 便宜又準）：sui-frontend 在 spec 階段抓到 `<div>/<img>` 塞進既有 `<p className="nl-empty">` ＝block-in-inline 非法 HTML（瀏覽器會自動關 `<p>` 破版）→ 改 `<figure>`；`@keyframes nl-reveal` 已在 App.css+Leaderboard.css **雙重定義**（body 相同才沒爆）→ 新動畫用 `nl-clam-open`；`mintPhase==='done'` 根本沒有對應 render site（status block 是 `{status &&}` gate 不是 mintPhase）→ spec 明寫插入位置。frontend-design 抓 `saturate(0.92)`＝8% no-op、卡通內建 drop-shadow + 加 halo＝雙陰影 slop tell、logo_2 太炫不該放「還沒人上榜」的冷清空狀態。**全 patch 進 spec → 4 個 SDD subagent 照抄即正確**。
