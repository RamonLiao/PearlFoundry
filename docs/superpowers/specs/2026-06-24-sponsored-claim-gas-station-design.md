...
- **SDK verified (installed):**
  ...
  - **Execution path (LIVE-PROVEN on testnet, sui 2.19 grpc):** `grpc.core.executeTransaction({ transaction: <Uint8Array bcs bytes>, signatures: [<base64 sig>, ...], include: { effects: true } })`. Note: method is on `.core` (top-level `grpc.executeTransaction` does NOT exist); `transaction` is the **Uint8Array** bytes (not base64 string — string throws); `signatures` are **base64** strings; result is a discriminated union `{ $kind: 'Transaction' | 'FailedTransaction', Transaction: { digest, effects: { status }, ... } }` (`$kind==='Transaction'` ⇒ success). **JSON-RPC `executeTransactionBlock` is NOT used** (Quorum Driver disabled under Protocol 126, deactivation 2026-07-31). `dryRunTransactionBlock` (used by `/quote`) unaffected.
  - **Sponsored dual-sig mechanism LIVE-PROVEN (2026-06-24):** built a tx with `setSender(holder)+setGasOwner(sponsor)+setGasPayment(sponsor coin)+setGasBudget`, signed by two distinct keypairs (holder + sponsor via `sui keytool sign`), executed via gRPC → on-chain Success (digest `9ABVbVc6…`: Sender=holder, Gas Owner=sponsor, distinct). C1 fully de-risked. **Caveat:** this used keypairs (which sign verbatim); it does NOT cover C2 (whether the *browser wallet* mutates gasData) — still browser-only.
...
- **fallback branches by failure point (MUST)** — pre-popup failures (503 `NO_SPONSOR`, 502 dry-run, sponsor-sign error, `NO_SPONSOR_GAS`) → fall back **silently** to self-pay (one wallet popup total; optional quiet `nl-note` "Gas sponsor unavailable — you'll cover gas this time"). Failure **after** holder already signed (sign-reject, **byte-equality mismatch** = `verify` phase, or execute error) → surface `nl-status--err`, **do NOT auto re-sign** (that's the confusing double-prompt). `BYTE_MISMATCH` additionally sets `forceSelfPay` so the next click goes straight to self-pay. When self-pay is used, drop the gas-free chip on next render so realized state isn't a lie.
