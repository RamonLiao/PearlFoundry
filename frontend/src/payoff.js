 ...
 *
 * `leftover` (optional, default 0) lifts the whole staircase: the unspent net principal sits in
 * the manager's BalanceManager and is reclaimed at claim, so the real payout is
 * `leftover + qty×(#strikes below settlement)` — floor = leftover, cap = leftover + qty×legs.
 *
 * @param {{lower: bigint|string|number, upper: bigint|string|number,
 *          step: bigint|string|number, qtyPerLeg: bigint|string|number,
 *          leftover?: bigint|string|number}} p
 */
export function computePayoffCurve(p) {
  ...
  const leftover = BigInt(p.leftover ?? 0);

  if (leftover < 0n) throw new Error('payoff: leftover must be >= 0');
  ...
  for (let k = 0n; k < legsBig; k++) {
    ...
    const before = leftover + qty * k;        // payout just below this strike (floor = leftover)
    const after = leftover + qty * (k + 1n);  // payout once this leg pays
  ...
  return {
    ...
    baseline: Number(leftover),
    maxPayout: Number(leftover + qty * legsBig),
    strikes,
    points,
  };
}
